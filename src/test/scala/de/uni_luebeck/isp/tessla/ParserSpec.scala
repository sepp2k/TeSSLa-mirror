...
class ParserSpec extends FlatSpec with Matchers {
  "A tessla parser" should "parse a simple definition" in {
    ...
    result should matchPattern {
      case Success(_, Spec(List(Def("foo", UnresolvedTerm("bar", ToBeInferred)))), _, _) =>}
  }
  ...
  it should "parse a definition including an application" in {
    ...
    result should matchPattern {
      case Success(_, Spec(List(
          Def("foo", App(UnresolvedFunction("bar"),
              List(UnresolvedTerm("baz", ToBeInferred)), List(), ToBeInferred)))), _, _) =>}
  }
  ...
  it should "parse a definition including an application with named arguments" in {
    ...
    result should matchPattern {
      case Success(_, Spec(List(
          Def("foo", App(UnresolvedFunction("bar"),
              List(), List(Def("arg", UnresolvedTerm("baz", ToBeInferred))), ToBeInferred)))), _, _) =>}
  }
  ...
  it should "parse a definition including a type ascription" in {
    ...
    result should matchPattern {
      case Success(_, Spec(List(
          Def("foo", TypeAscr(UnresolvedTerm("bar", ToBeInferred), UnresolvedPrimitiveType("int"))))), _, _) =>}
  }
