...
object CompiletimeExterns {
  ...
  val map_add_extern = Core.ExternExpression(
    ...
    List(
      (StrictEvaluation, Core.InstantiatedType("Map", List(Core.TypeParam(Identifier("A")), Core.TypeParam(Identifier("B"))))),
      (StrictEvaluation, Core.TypeParam(Identifier("A"))),
      (StrictEvaluation, Core.TypeParam(Identifier("B")))
    ),
    Core.InstantiatedType("Map", List(Core.TypeParam(Identifier("A")), Core.TypeParam(Identifier("B")))),
    "Map_add"
  )
  ...
  val reifiers = Map[String, Reifier](
    ...
    "List" -> ((value, typeArgs) => value.asInstanceOf[List[Any]] match {
      case a :: as => (ArraySeq((a, typeArgs.head), (as, Core.InstantiatedType("List", typeArgs))), app(list_prepend_extern, typeArgs))
    ...
    "Set" -> ((value, typeArgs) => {
      ...
      if (set.isEmpty)
        ...
        (ArraySeq((set.tail, Core.InstantiatedType("Set", typeArgs)), (set.head, typeArgs.head)), app(set_add_extern, typeArgs))
    }),
    "Map" -> ((value, typeArgs) => {
      ...
      if (map.isEmpty)
        ...
        (ArraySeq((map.tail, Core.InstantiatedType("Map", typeArgs)), (map.head._1, typeArgs.head), (map.head._2, typeArgs(1))),
