...
object IntermediateCodeTypeInference {
    ...
    impLanExpr match {
      ...
      case StructValue(vals) => {
        val orderedVals = vals.toSeq.sortWith {
          case ((n1, _), (n2, _)) => IntermediateCodeUtils.structComparation(n1, n2)
        }
  ...
  def generateCodeWithCasts(
    ...
    retType: Option[ImpLanType] = scala.None
  ): Seq[ImpLanStmt] = {
    stmts.map {
      case expr: ImpLanExpr => castExpression(expr, scala.None, varTypes)
      case If(guard, stmts, elseStmts) =>
        If(
          guard.map { _.map(e => castExpression(e, scala.Some(BoolType), varTypes)) },
      ...
      case Assignment(lhs, rexpr, defVal, typ) =>
        Assignment(lhs, castExpression(rexpr, scala.Some(typ), varTypes), defVal, typ)
      case FinalAssignment(lhs, defVal, typ, ld) =>
        FinalAssignment(lhs, castExpression(defVal, scala.Some(typ), varTypes), typ, ld)
      case ReturnStatement(expr) if retType.isDefined =>
        ReturnStatement(castExpression(expr, scala.Some(retType.get), varTypes))
  ...
  def castExpression(exp: ImpLanExpr, target: Option[ImpLanType], varTypes: Map[String, ImpLanType]): ImpLanExpr = {
    val innerExp: ImpLanExpr = exp match {
      case CastingExpression(e, from, target) =>
        CastingExpression(castExpression(e, scala.Some(from), varTypes), from, target)
      case FunctionCall(name, params, typeHint) =>
        FunctionCall(
          name,
          params.zip(typeHint.argsTypes).map { case (e, t) => castExpression(e, scala.Some(t), varTypes) },
          typeHint
        )
      case LambdaApplication(exp, params) =>
        typeInference(exp, varTypes) match {
          case FunctionType(argsTypes, _) =>
            LambdaApplication(
              castExpression(exp, scala.None, varTypes),
              params.zip(argsTypes).map { case (e, t) => castExpression(e, scala.Some(t), varTypes) }
            )
      ...
      case TernaryExpression(guard, e1, e2) => {
        ...
        TernaryExpression(
          guard.map { _.map { c => castExpression(c, scala.Some(BoolType), varTypes) } },
        ...
        LambdaExpression(
          ...
          generateCodeWithCasts(
            ...
            IntermediateCodeUtils
              .getVariableMap(
                body,
                varTypes.view.mapValues { a => (a, scala.None, false) }.toMap ++ argNames
                  .zip(argsTypes)
                  .map { case (a, b) => (a, (b, scala.None, false)) }
                  .toMap
              )
              .view
