...
class ASTPreprocessor(duplicateInlineCandidates: Boolean) extends TranslationPhase[Core.Specification, Core.Specification] {
  ...
  override def translate(spec: Core.Specification): Result[Core.Specification] = {
    ...
    def getExpRef(exp: ExpressionArg, add: mutable.Map[Identifier, DefinitionExpression], scope: Map[Identifier, DefinitionExpression]):  ExpressionArg = {
      exp match {
        case exp : Expression =>
          val newID = freshId()
          add += (newID -> flattenDefExp(exp, add, scope))
          ExpressionRef(newID, exp.tpe, Location.unknown)
        case e: ExpressionRef if duplicateInlineCandidates && mutability_check.ASTTransformation.isInlineExpression(e, scope) =>
          val newID = freshId()
          val newExp = mutability_check.ASTTransformation.getInlineExpression(e, scope).asInstanceOf[DefinitionExpression]
          add += (newID -> flattenDefExp(newExp, add, scope))
          ExpressionRef(newID, exp.tpe, Location.unknown)
        case e: ExpressionRef => e
      }
    }

    def flattenDefExp(exp: DefinitionExpression, add: mutable.Map[Identifier, DefinitionExpression], scope: Map[Identifier, DefinitionExpression]) : DefinitionExpression = {
      exp match {
        case FunctionExpression(typeParams, params, body, result, location) => {
          val newBody = mutable.Map.from(flattenDefs(body, scope ++ body))
          val newResult = flattenExpArg(result, newBody, scope ++ body)
        ...
        case ApplicationExpression(applicable, args, location) =>
          applicable match {
            case ExternExpression(_, _, _, "lift", _)
              ...
              ApplicationExpression(applicable, args.dropRight(1).map(getExpRef(_, add, scope)) :+ args.last, location)
            case _: ExternExpression |
                 TypeApplicationExpression(_: ExternExpression, _, _) =>
              ApplicationExpression(applicable, args.map(getExpRef(_, add, scope)), location)
            case _ =>
              ApplicationExpression(getExpRef(applicable, add, scope), args.map(getExpRef(_, add, scope)), location)
          }
        case TypeApplicationExpression(applicable, typeArgs, location) =>
          TypeApplicationExpression(flattenExpArg(applicable, add, scope), typeArgs, location)
        case RecordConstructorExpression(entries, location) =>
          val newEntries = entries.map{case (n, (e, l)) => (n, (getExpRef(e, add, scope), l))}
          RecordConstructorExpression(newEntries, location)
        case RecordAccessorExpression(name, target, nameLocation, location) =>
          RecordAccessorExpression(name, getExpRef(target, add, scope), nameLocation, location)
        case _ => exp
      }
    }

    def flattenExpArg(exp: ExpressionArg, add: mutable.Map[Identifier, DefinitionExpression], scope: Map[Identifier, DefinitionExpression]) : ExpressionArg = {
      exp match {
        case exp: Expression => flattenDefExp(exp, add, scope)
        case exp: ExpressionRef => getExpRef(exp, add, scope)
      }
    }

    def flattenDefs(defs: Map[Identifier, DefinitionExpression], scope: Map[Identifier, DefinitionExpression]): Map[Identifier, DefinitionExpression] = {
      val newDefs : mutable.Map[Identifier, DefinitionExpression] = mutable.Map()
      val transDefs = defs.map{case (id, defExp) => (newIDFromOld(id) -> flattenDefExp(defExp, newDefs, scope))}
    ...
    val newDefs = flattenDefs(definitions, definitions)
