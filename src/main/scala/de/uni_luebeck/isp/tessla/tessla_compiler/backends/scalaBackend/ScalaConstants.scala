...
object ScalaConstants {
  ...
  def valueTranslation(v: ImpLanVal): String = {
    v match {
      ...
      case StringValue(value) =>
        s""""${value.replaceAllLiterally("\"", "\\\"")}"""" //TODO: Find better solution, re-escaping all special chars
  ...
  def builtinFunctionCallTranslation(
    ...
    name match {
      case "__[TC]output__" =>
        s"outputVar(${ScalaIOHandling
          .getParseExpressionToString(typeHint.argsTypes(0), args(0))}, ${args(1)}, ${args(2)}, ${args(3)}, ${args(4)})"
