...
import de.uni_luebeck.isp.tessla.tessla_compiler.IntermediateCode.{Assignment, FinalAssignment, ImpLanExpr, ImpLanStmt, ImpLanType, ImpLanVal}
...
class JavaBackend extends BackendInterface("de/uni_luebeck/isp/tessla/tessla_compiler/JavaSkeleton.java") {
  ...
  def translateExpression(e: ImpLanExpr) : String = e match {
      ...
      case IntermediateCode.FunctionCall(name, params) => s"$name(${params.map(translateExpression).mkString(", ")})"
      ...
      case IntermediateCode.Equal(a, b) => s"${translateExpression(a)} == ${translateExpression(b)}"
