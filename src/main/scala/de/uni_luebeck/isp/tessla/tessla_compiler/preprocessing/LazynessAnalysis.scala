...
import de.uni_luebeck.isp.tessla.TesslaAST.LazyEvaluation
...
class LazynessAnalysis extends TranslationPhase[ExtendedSpecification, ExtendedSpecification] {
  ...
  override def translate(extSpec: ExtendedSpecification): Result[ExtendedSpecification] = {
    ...
    def extractSubIDs(arg: ExpressionArg): Set[Identifier] = {
      arg match {
        case ApplicationExpression(applicable, args, _) => args.foldLeft[Set[Identifier]](extractSubIDs(applicable)){case (s, i) => s ++ extractSubIDs(i)}
        case TypeApplicationExpression(applicable, _, _) => extractSubIDs(applicable)
        case RecordConstructorExpression(entries, _) => entries.map(_._2._1).foldLeft[Set[Identifier]](Set()){case (s, i) => s ++ extractSubIDs(i)}
    ...
    def examineLazynessCandidate(e: ExpressionArg, scope: Map[Identifier, DefinitionExpression], stack: Set[Identifier] = Set()): Unit = {
      extractSubIDs(e).foreach(examineLazynessCandidateID(_, scope, stack))
    }

    def examineLazynessCandidateID(id: Identifier, scope: Map[Identifier, DefinitionExpression], stack: Set[Identifier] = Set()): Unit = {
    ...
    def mapDefinitionsForLazyness(m: Map[Identifier, DefinitionExpression], scope: Map[Identifier, DefinitionExpression]) : Unit = {
