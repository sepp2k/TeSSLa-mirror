...
object RuntimeExterns {
  ...
  def binIntOp[A[+ _] : Monad](f: (BigInt, BigInt) => Any) = strict(l =>
    f(l(0).asInstanceOf[BigInt], l(1).asInstanceOf[BigInt]).pure[A])

  def unaryIntOp[A[+ _] : Monad](f: BigInt => Any) = strict(l => f(l(0).asInstanceOf[BigInt]).pure[A])

  def binBoolOp[A[+ _] : Monad](f: (Boolean, Boolean) => Any) = strict(l => f(l(0).asInstanceOf[Boolean], l(1).asInstanceOf[Boolean]).pure[A])

  def binFloatOp[A[+ _] : Monad](f: (Double, Double) => Any) = strict(l => f(l(0).asInstanceOf[Double], l(1).asInstanceOf[Double]).pure[A])

  def binAnyOp[A[+ _] : Monad](f: (Any, Any) => Any) = strict(l => f(l(0), l(1)).pure[A])

  def unaryFloatOp[A[+ _] : Monad](f: Double => Any) = strict(l => f(l(0).asInstanceOf[Double]).pure[A])
  ...
  def commonExterns[A[+ _] : Monad]: Map[String, Extern[A]] = Map(
    ...
    "fgt" -> binFloatOp(_ > _),
    "fgeq" -> binFloatOp(_ >= _),
    "and" -> ((arguments: ArraySeq[A[Any]]) => arguments(0).flatMap { cond =>
      propagateSingle(cond) { c =>
        if (c.asInstanceOf[Boolean]) arguments(1) else false.pure[A]
      }
    }),
    "or" -> ((arguments: ArraySeq[A[Any]]) => arguments(0).flatMap { cond =>
      propagateSingle(cond) { c =>
    ...
    "Map_get" -> strict(propagate(_) { arguments =>
      arguments(0).asInstanceOf[Map[Any, Any]].getOrElse(arguments(1), RuntimeError("No such element.")).pure[A]
    }),
    ...
    "Map_fold" -> ((arguments: ArraySeq[A[Any]]) => arguments(0).flatMap(propagateSingle(_) { map =>
    ...
    "Set_fold" -> ((arguments: ArraySeq[A[Any]]) => arguments(0).flatMap(propagateSingle(_) { map =>
      arguments(2).flatMap(propagateSingle(_) { f =>
