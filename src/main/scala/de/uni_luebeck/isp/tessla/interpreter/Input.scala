...
object Input {
  ...
  case class TimeUnit(loc: Location, timeUnit: de.uni_luebeck.isp.tessla.TimeUnit.TimeUnit){
    override def toString = timeUnit.toString
  }

  case class Event(loc: Location, timeStamp: BigInt, name: Identifier, value: TesslaCore.LiteralValue){
