...
object ImplicitConstraints {
  ...
  implicit def implicitConstraintOrA[A, B <: ConstraintOr](implicit a: A): ||:[A, B] = new ||:[A, B] {
    override def switch[C](fs: FS[C]): C = fs.head(a)
  }

  implicit def implicitConstraintOrB[A, B, C <: ConstraintOr](implicit b: B ||: C): ||:[A, B ||: C] = new ||:[A, B ||: C] {
    override def switch[D](fs: FS[D]): D = b.switch(fs.tail)
  }
