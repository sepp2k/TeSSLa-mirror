...
package object util {
  ...
  def mapValues[K, V1, V2](map: Map[K, V1])(f: V1 => V2): Map[K, V2] = map.map { case (k, v) => k -> f(v) }
  def mapValues[K, V1, V2](map: MMap[K, V1])(f: V1 => V2): MMap[K, V2] = map.map { case (k, v) => k -> f(v) }
}