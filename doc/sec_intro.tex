...
\begin{enumerate}
	\item \textit{The difference of the values written to a buffer of size 5 and the values deleted from it should never exceed 5.}
...
\begin{enumerate}
	\item \textit{There are three cores and only the events that happened on core one and two are important.}
	\item \textit{If an event with a value smaller than zero occurs, create an event with the value zero, else create an event with the value that occurred.}
\end{enumerate}
