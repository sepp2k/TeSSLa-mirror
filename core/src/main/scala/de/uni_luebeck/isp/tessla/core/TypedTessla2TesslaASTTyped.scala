...
class TypedTessla2TesslaASTTypedWorker(
  ...
  ): Option[Typed.ExpressionArg] = {
    exp match {
      case InputStream(name, streamType, typeLoc, loc) =>
        val id = Typed.Identifier(Ior.Left(name), loc)
        val typ = toType(streamType, typeLoc)
        Some(Typed.ExpressionRef(id, typ, loc))
      case Parameter(_, _, _) =>
        // Previous phases use definitions like `def x$123 = param x$123` to include the parameters
        // in the set of definitions. This is obsolete at this point, and needs to be removed since the
        // ConstantEvaluator would detect it as infinite recursion otherwise
        None
      case Macro(typeParameters, parameters, body, _, _, result, loc, _) =>
        val translatedTypeParams = typeParameters.map(toIdentifier(_, Location.unknown)).toList
        val translatedParameters = parameters.map {
          case (eval, typ) =>
            val id = toIdentifier(typ.id, typ.loc)
            val ev = eval.getOrElse(
              if (typ.parameterType.isStreamType) TesslaAST.LazyEvaluation
              else TesslaAST.StrictEvaluation
            )
            val t = toType(typ.parameterType)
            (id, ev, t)
        }.toList
        val translatedResult = Typed.ExpressionRef(
          toIdentifier(result.id, Location.unknown),
          lookupType(result.id, body),
          result.loc
        )
        val translatedBody = translateEnv(body)
        Some(
          Typed.FunctionExpression(translatedTypeParams, translatedParameters, translatedBody, translatedResult, loc)
        )
      case Variable(id, loc) =>
        Some(Typed.ExpressionRef(toIdentifier(id, lookup(env, id).loc), lookupType(id, env), loc))
      case MacroCall(macroID, macroLoc, typeArgs, args, loc) =>
        ...
        Some(Typed.ApplicationExpression(callable, translatedArgs, loc))
      case Extern(name, _, _, _, loc) => // TODO: support reference implementation
        val t = toType(typeInfo)
        Some(Typed.ExternExpression(name, t, loc))
      case MemberAccess(receiver, member, memberLoc, loc) =>
        val target = Typed.ExpressionRef(
          toIdentifier(receiver.id, receiver.loc),
          lookupType(receiver.id, env)
        )
        Some(Typed.RecordAccessorExpression(member, target, memberLoc, loc))
      case ObjectLiteral(members, loc) =>
        val translatedMembers = mapValues(members) {
          case IdLoc(id, loc) =>
            val ref = Typed.ExpressionRef(toIdentifier(id, loc), lookupType(id, env))
            (ref, loc)
        }
        Some(Typed.RecordConstructorExpression(translatedMembers, loc))
      case Literal(value, loc) => Some(translateLiteralValue(value, loc))
    }
  }
