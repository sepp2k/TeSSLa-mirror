...
object ScalaConstants {
  ...
  def valueTranslation(v: ImpLanVal): String = {
    v match {
      ...
      case EmptyMutableQueue(t)      => typeTranslation(MutableQueueType(t)) + "()"
      case EmptyImmutableQueue(t)    => typeTranslation(ImmutableQueueType(t)) + "()"
  ...
  def builtinFunctionCallTranslation(
    ...
    name match {
      ...
      case "__Queue_empty__"                                                  => typeTranslation(typeHint.retType) + "()"
