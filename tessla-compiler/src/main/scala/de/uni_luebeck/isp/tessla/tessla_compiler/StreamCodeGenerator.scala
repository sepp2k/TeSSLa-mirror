 ...
 * @param nonStreamCodeGenerator Corresponding [[NonStreamCodeGenerator]] used for expression translation
 * @param myControlFlowAnalysis Corresponding [[ControlFlowAnalysis]] used to determine additional guards
 */

class StreamCodeGenerator(nonStreamCodeGenerator: NonStreamCodeGenerator, myControlFlowAnalysis: ControlFlowAnalysis) {
  ...
  def translateExternSignalExpression(
    ...
    e.name match {
      ...
      //TODO: Check compliance with control flow analysis
      case "count" =>
  ...
  /**
   * Function generating additional guard around stream definition to avoid unnecessary stream evaluations
   * @param id Id for which the additional guard shall be generated
   * @return Additional guard
   */
  private def getAdditionalGuard(id: Identifier): Seq[Seq[ImpLanExpr]] = {
