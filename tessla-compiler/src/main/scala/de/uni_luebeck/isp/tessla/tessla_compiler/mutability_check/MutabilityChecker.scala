...
import de.uni_luebeck.isp.tessla.tessla_compiler.{DefinitionOrdering, ExtendedSpecification}
...
case class TesslaCoreWithMutabilityInfo(
  spec: ExtendedSpecification,
  idTypes: (Identifier, Map[Identifier, DefinitionExpression]) => Type,
  addDeps: Map[Identifier, Set[Identifier]],
  impCheck: ImplicationChecker
) {
 ...
 */
object MutabilityChecker extends TranslationPhase[ExtendedSpecification, TesslaCoreWithMutabilityInfo] {

  override def translate(espec: ExtendedSpecification): TranslationPhase.Result[TesslaCoreWithMutabilityInfo] = {
    val spec = espec.spec
    ...
    val cfAnalysis = new ControlFlowAnalysis(espec)
    ...
    def processDependencies(id: Identifier, dep: ExpressionFlowAnalysis.IdentifierDependencies): Unit = {
      ...
      if (dep.reads != Set() || dep.writes != Set() || dep.reps != Set()) {
        ...
        cfDependencies += (id -> dep.deps
          .map { d => if (cfDependencies.contains(d)) cfDependencies(d) else Set(d) }
          .foldLeft[Set[Identifier]](Set()) { case (c, s) => c ++ s })
        if (dep.pass.nonEmpty) {
          passDependencies += (id -> dep.pass
            .map { d => if (passDependencies.contains(d)) passDependencies(d) else Set(d) }
            .foldLeft[Set[Identifier]](Set()) { case (c, s) => c ++ s })
        }
    ...
    def getIDsIfInlineExpression(e: ExpressionArg, scope: Map[Identifier, DefinitionExpression]): Set[Identifier] = {
      e match {
        case ApplicationExpression(TypeApplicationExpression(e, _, _), args, _) => {
          e match {
            case ExternExpression(op, _, _) if Set("ite", "staticite", "and", "or").contains(op) =>
    ...
    def processDefinitions(
      defs: Seq[(Identifier, DefinitionExpression)],
      ...
      def execOnFuncExpr(e: ExpressionArg): Unit = {
        e match {
          case FunctionExpression(_, pars, body, _, _) =>
            ...
            processDefinitions(DefinitionOrdering.order(body), scope ++ body)
    ...
    processDefinitions(DefinitionOrdering.order(spec.definitions), spec.definitions)

    val repsMap: Map[Identifier, Set[(Identifier, Identifier)]] = invRepsMap
      .flatMap {
        case (k, v) =>
      ...
      .groupBy(_._1)
      .view
      .mapValues { vs => vs.map(_._2).toSet }
      .toMap
    ...
    def cleanParent(
      ...
      val writes =
        if (beat.isEmpty) writeMap.getOrElse(node, Set()).filter(!incompatibleVars(_, caller))
        else writeMap.getOrElse(node, Set())

      //If there are writes or replicating lasts return false, else climb down over children
      if (
        writes.size > (if (beat.isEmpty) 1 else 0) ||
        (beat.nonEmpty && !repsMap
          .getOrElse(node, Set())
          .filter(_._1 == beat.head)
          .exists(c => impCheck.freqImplication(beat.head, c._2, false)))
      ) {
        ...
        if (childClean) {
          ...
          val ret = repParents
            .map(cleanParent(_, node +: beat, node, origWrite))
            .foldLeft[(Set[Identifier], Boolean)]((childReads, childClean)) {
              case ((s1, b1), (s2, b2)) => (s1.union(s2), b1 && b2)
            }
    ...
    def cleanChild(
      ...
      val cr: (Set[Identifier], Boolean) =
        if (beat.nonEmpty) cleanChild(node, beat.drop(1), caller, origWrite) else (Set(), false)

      //check if aliasing constraint is fulfilled and go down recursively otherwise
      val pr: (Set[Identifier], Boolean) =
        if (
          beat.isEmpty || writeMap.getOrElse(node, Set()).nonEmpty ||
          repsMap
            .getOrElse(caller, Set())
            .filter(_._1 == node)
            .exists(c => !impCheck.freqImplication(beat.head, c._2, false))
        ) {
          (Set(), false)
    ...
    def dealChilds(
      ...
      if (childs.nonEmpty) {
        ...
        childs.map(c => cleanChild(c, beat, caller, origWrite)).reduce[(Set[Identifier], Boolean)] {
          case ((s1, b1), (s2, b2)) => (s1.union(s2), b1 && b2)
        }
    ...
    val readBeforeWrites: collection.mutable.ArrayBuffer[(Identifier, Identifier, Identifier)] =
      collection.mutable.ArrayBuffer()
    writeMap.foreach {
      case (writtenNode, writeNodes) =>
    ...
    readBeforeWrites.flatMapInPlace {
      case (i, j, m) =>
    ...
    if (readBeforeWrites.nonEmpty) {
      val z3H = new Z3Handler(
        edges.toSet ++ cfAnalysis.getAddOrderingConstraints,
        readBeforeWrites.toSet,
        inlinings.toMap,
        variableFamilies
      )
    ...
    val addDeps = readBeforeWrites
      ...
      .groupBy(_._1)
      .view
      .mapValues(e => e.map(x => x._2).toSet)
      .toMap
    ...
    def targetVarType(id: Identifier, scope: Map[Identifier, DefinitionExpression]): Type = {
      ...
      if (origType.isInstanceOf[FunctionType]) {
        if (scope.contains(id)) {
          scope(id) match {
            case FunctionExpression(typeParams, params, body, result, _) =>
              FunctionType(
                typeParams,
                params.map { case (id, ev, _) => (ev, targetVarType(id, scope)) },
                targetVarType(result.asInstanceOf[ExpressionRef].id, scope ++ body)
              )
        ...
        (mutabilityCheckRelevantType(origType) || mutabilityCheckRelevantStreamType(origType)) && !immutVars.contains(
          id
        )
      ) {
    ...
    Success(
      TesslaCoreWithMutabilityInfo(
        espec.copy(spec = spec.copy(in = in, definitions = definitions, out = out)),
        targetVarType,
        addDeps,
        impCheck
      ),
  ...
  def mutabilityCheckRelevantType(tpe: Type): Boolean = {
    ...
    tpe match {
      case InstantiatedType("Map", _, _) | InstantiatedType("Set", _, _) | InstantiatedType("List", _, _) |
          InstantiatedType("Queue", _, _) =>
        true
