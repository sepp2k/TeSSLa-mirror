...
* Add support for multi-threading to instrumentations generated by the observation annotations. The `--observations-pthread` option creates thread-safe instrumentation code snippets using pthread mutexes guarding every access to the trace log file. The new annotation `@ThreadId` creates an event carrying the current pthread thread ID for every generated on any other stream.
