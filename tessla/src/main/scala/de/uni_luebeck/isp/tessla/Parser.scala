  ...
  object Parsers extends Parsers {
    ...
    def comparison: Parser[Ast.Expr] = additiveExpr ~ ((EQ | LT | GT | LEQ | GEQ | NEQ) ~ additiveExpr).* ^^! {
      case (loc, (lhs, rhss)) => rhss.foldLeft(lhs) {
        case (l,(op, r)) =>
          if(op.value == NEQ) {
            val eq = Ast.ExprApp(Ast.Identifier("eq", SourceLoc(op.loc)), List(Ast.PosArg(l), Ast.PosArg(r)), SourceLoc(loc))
            Ast.ExprApp(Ast.Identifier("signalNot", SourceLoc(op.loc)), List(Ast.PosArg(eq)), SourceLoc(loc))
          } else {
