...
class Interpreter(val spec: Core.Specification) extends StreamEngine(ConstantEvaluator.Record(Map())) {
  ...
  lazy val definitions: RuntimeEvaluator.Env =
    mapValues(inStreams)(x => Lazy(x._1)) ++ spec.definitions.map(d =>
          ...
          stream.addListener {
            case Some(value) =>
              if (!stopped) {
                ...
                value match {
                  case ConstantEvaluator.RuntimeError(msg) => throw Errors.RuntimeError(msg)
          ...
          interpreter.inStreams.get(event.stream.name) match {
            case Some((inStream, elementType)) =>
              ...
              RuntimeTypeChecker
                ...
                .foreach(error => throw Errors.mkTesslaError("input " + event.stream.name + ": " + error, event.loc))
